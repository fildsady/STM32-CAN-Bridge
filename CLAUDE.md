...
- **LL driver เท่านั้น ห้ามใช้ HAL** — CAN ไม่มี LL ใช้ register ตรง โฟลเดอร์ชื่อ HAL_Driver เพราะ ST รวม LL ไว้ด้วย ไม่ต้องเปลี่ยนชื่อ
- **FreeRTOS เสมอ ห้าม bare-metal** — แม้แค่ blink LED
...
- **Flash write: erase ก่อน + disable IRQ** — sector อาจไม่ blank (0x00 ไม่ใช่ 0xFF) ต้อง erase ก่อน write + `__disable_irq()` ระหว่าง flash operation
- **อย่าทิ้ง debug test code** — โดยเฉพาะ code ที่ erase flash sector ทุก boot
- **Memory save 2 ที่** — ทั้ง .claude/ memory + CLAUDE.md ใน repo ทุกครั้ง
- **อย่าถาม "ไว้ session หน้าไหม"** — ทำเลยเสมอ
